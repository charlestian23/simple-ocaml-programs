...
let rec encode_helper (word : string) (increment : int) =
  ...
  else (Char.escaped (increment_letter word.[0] increment)) ^ (encode_helper (String.sub word 1 (length - 1)) increment)

let encode (word : string) (offset : int) =
  let increment = offset mod 26 in (encode_helper word increment)
